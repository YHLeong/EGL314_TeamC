...
sensor_label = tk.Label(root, text="Press Start to Begin", font=("Arial", 16))
sensor_label.grid(row=1, column=0, pady=5)

pressed_label = tk.Label(root, text="Pressed Sensors: -", font=("Arial", 14))
pressed_label.grid(row=2, column=0, pady=5)

score_label = tk.Label(root, text="Score: 0", font=("Arial", 14))
score_label.grid(row=3, column=0, pady=5)
...
#Game Start
def start_game():
    global current_level, current_sequence, player_progress, mistakes, score
    current_level = "Easy"
    score = 0
    ...
    pressed_label.config(text="Pressed Sensors: -")
    current_sequence = generate_sequence(current_level)
    root.after(2000, lambda: display_sequence(current_sequence))
