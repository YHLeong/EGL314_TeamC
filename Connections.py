...
SENSOR_MAP = {1: 22, 2: 6, 3: 19, 4: 16, 5: 20, 6: 21}
...
for i, (sensor_num, pin) in enumerate(sorted(SENSOR_MAP.items())):

    label = tk.Label(root, text=f"Sensor {sensor_num} (Pin {pin}): Checking...", font=("Arial", 16))

    label.grid(row=i, column=0, pady=5)  # Use row=i to avoid overlapping
